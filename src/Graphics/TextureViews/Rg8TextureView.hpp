...
namespace texture_view {
    class Rg8TextureView : public ITextureView {
        static constexpr int PIXEL_STRIDE = 2;

    public:
        Rg8TextureView(const uint8_t* pixels, uint32_t width, uint32_t height, uint32_t pitch)
        ...
        DOOB_NODISCARD glm::vec4 Read(uint32_t x, uint32_t y) const override {
            ...
            const float g = static_cast<float>(start[1]) / 255.0f;

            return { r, g, 1.0f, 1.0f };
        }

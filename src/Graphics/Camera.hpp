...
public:
    ...
    DOB_FORCEINLINE void LookDir(const glm::vec3& ray, float fov_degrees, const glm::vec3& up = { 0, 1, 0 }) {
    ...
    DOB_NODISCARD DOB_FORCEINLINE glm::vec3 GetRay(glm::vec2 ndc) const {
        return glm::normalize((m_right * ndc.x + m_up * ndc.y) + m_forward * m_focal_length);
    }

    DOB_NODISCARD DOB_FORCEINLINE glm::vec3 GetPosition() const { return m_position; }
    DOB_FORCEINLINE void SetPosition(const glm::vec3& position) { m_position = position; }

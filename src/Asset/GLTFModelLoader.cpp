...
namespace model_loader {
    ...
    ModelData GLTFModelLoader::Load(const std::filesystem::path& filepath) {
        ...
        auto castOptionalTexture = [](const auto& optional) -> auto {
            return optional.has_value() ? std::make_optional(static_cast<uint32_t>(optional.value().textureIndex))
                                        : std::nullopt;
        };
        ...
        for (const fastgltf::Scene& gltfScene : asset.scenes) {
            ...
            for (size_t value : gltfScene.nodeIndices) {
        ...
        for (fastgltf::Node& gltfNode : asset.nodes) {
            ...
            for (size_t value : gltfNode.children) {
                node.children.push_back(value);
            }

            node.transform = std::visit(
                fastgltf::visitor{ [](fastgltf::TRS trs) {
                                      Transform transform = {};
                                      transform.position = { trs.translation.x(), trs.translation.y(),
                                          trs.translation.z() };
                    ...
                    [](fastgltf::math::fmat4x4 trs) {
                        auto decompose_transform = [](const glm::mat4& transform, glm::vec3& translation,
                            ...
                            if (glm::epsilonNotEqual(
                                    local_matrix[0][3], static_cast<float>(0), glm::epsilon<float>()) ||
                                glm::epsilonNotEqual(
                                    local_matrix[1][3], static_cast<float>(0), glm::epsilon<float>()) ||
                                glm::epsilonNotEqual(
                                    local_matrix[2][3], static_cast<float>(0), glm::epsilon<float>())) {
                        ...
                        glm::vec3 rotation = {};
                        decompose_transform(
                            *reinterpret_cast<glm::mat4*>(&trs), transform.position, rotation, transform.scale);
        ...
        for (const fastgltf::Mesh& gltfMesh : asset.meshes) {
            ...
            for (const fastgltf::Primitive& primitive : gltfMesh.primitives) {
                meshGroup.meshes.emplace_back(
                    castOptionalU32(primitive.materialIndex), modelData.meshGroups.size(), meshGroup.meshes.size());
            }
        ...
        for (const fastgltf::Material& gltfMaterial : asset.materials) {
            ...
            material.albedoColor = { gltfMaterial.pbrData.baseColorFactor.x(), gltfMaterial.pbrData.baseColorFactor.y(),
                gltfMaterial.pbrData.baseColorFactor.z(), gltfMaterial.pbrData.baseColorFactor.w() };
            ...
            material.emissiveFactor = { gltfMaterial.emissiveFactor.x(), gltfMaterial.emissiveFactor.y(),
        ...
        for (const fastgltf::Camera& gltfCamera : asset.cameras) {
            ...
            if (std::holds_alternative<fastgltf::Camera::Perspective>(gltfCamera.camera)) {
                const fastgltf::Camera::Perspective perspective =
                    std::get<fastgltf::Camera::Perspective>(gltfCamera.camera);
                camera.camera = Camera::Perspective{
                    .aspectRatio =
                        castOptionalType(perspective.aspectRatio, [](auto value) { return static_cast<float>(value); }),
                ...
                const fastgltf::Camera::Orthographic orthographic =
                    std::get<fastgltf::Camera::Orthographic>(gltfCamera.camera);
    ...
    ImageData GLTFModelLoader::ReadImageData(const ModelData& modelData, const Image& image) {
        ...
        uint8_t* data = stbi_load_from_memory(reinterpret_cast<const uint8_t*>(gltfImageData.data()),
            static_cast<int>(gltfImageData.size()), reinterpret_cast<int*>(&imageData.width),
            reinterpret_cast<int*>(&imageData.height), &numChannels, 4);

        if (!data) {
            std::println("Failed to open image file");
        }

        imageData.data.resize(static_cast<uint64_t>(imageData.width * imageData.height * 4));
        std::memcpy(imageData.data.data(), data, static_cast<uint64_t>(imageData.width * imageData.height * 4));
    ...
    MeshData GLTFModelLoader::ReadMeshData(const ModelData& modelData, const Mesh& mesh) {
        ...
        MeshData meshData = {};
        if (const fastgltf::Attribute* attribute = gltfPrimitive.findAttribute("POSITION");
            attribute != gltfPrimitive.attributes.end()) {
        ...
        if (const fastgltf::Attribute* attribute = gltfPrimitive.findAttribute("NORMAL");
            attribute != gltfPrimitive.attributes.end()) {
            const fastgltf::Accessor& accessor = asset.accessors[attribute->accessorIndex];
        ...
        if (const fastgltf::Attribute* attribute = gltfPrimitive.findAttribute("TANGENT");
            attribute != gltfPrimitive.attributes.end()) {
            const fastgltf::Accessor& accessor = asset.accessors[attribute->accessorIndex];
        ...
        if (gltfPrimitive.indicesAccessor.has_value()) {
            ...
            if (accessor.componentType == fastgltf::ComponentType::UnsignedInt) {
                fastgltf::iterateAccessorWithIndex<uint32_t>(
                    asset, accessor, [&](uint32_t value, size_t index) { meshData.indices[index] = value; }, adapter);
            } else if (accessor.componentType == fastgltf::ComponentType::UnsignedShort) {
                fastgltf::iterateAccessorWithIndex<uint16_t>(
                    asset, accessor,
                    [&](uint16_t value, size_t index) { meshData.indices[index] = static_cast<uint32_t>(value); },
                    adapter);
            } else if (accessor.componentType == fastgltf::ComponentType::UnsignedByte) {
                fastgltf::iterateAccessorWithIndex<uint8_t>(
                    asset, accessor,
                    [&](uint8_t value, size_t index) { meshData.indices[index] = static_cast<uint32_t>(value); },
                    adapter);
            }
        }

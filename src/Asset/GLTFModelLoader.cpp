    ...
    auto getGltfData = [&](this auto&& self, const fastgltf::DataSource& data) -> std::vector<std::byte> {
        return std::visit(fastgltf::visitor{
                              ...
                              [&](const fastgltf::sources::BufferView& source) -> std::vector<std::byte> {
                                  ...
                                  std::memcpy(
                                      ret.data(), bufferData.data() + bufferView.byteOffset, bufferView.byteLength);
                                  return ret;
                              },
    ...
    if (gltfPrimitive.indicesAccessor.has_value()) {
        ...
        if (accessor.componentType == fastgltf::ComponentType::UnsignedInt) {
            ...
            fastgltf::iterateAccessorWithIndex<uint16_t>(
                asset, accessor,
                [&](uint16_t value, size_t index) { meshData.indices[index] = static_cast<uint32_t>(value); }, adapter);
        } else if (accessor.componentType == fastgltf::ComponentType::UnsignedByte) {
            fastgltf::iterateAccessorWithIndex<uint8_t>(
                asset, accessor,
                [&](uint8_t value, size_t index) { meshData.indices[index] = static_cast<uint32_t>(value); }, adapter);
        }
    }
